...
Payment.prototype.sale = function(nonce) {
  ...
  Payments.update({_id: this._id}, {$set: {request: options, response: response}});

  if (response.success) {
    Payments.update({_id: this._id}, {$set: {authorizedAt: new Date()}});
    paymentRequest.logAuthorized(this);
  }

  console.log("[Payment] sale: ", Payments.findOne(this._id, {transform: null}));
