...
Meteor.startup(function customerStartup() {
  PaymentRequests.find({ 'customerId' : { $exists : false }}).observe({
    'added' : function(payReq) {
      ...
      var customer = Customers.createIfNotExisted(bill.email, bill.recipient.firstName, bill.recipient.lastName);
      PaymentRequests.update({ _id : payReq._id }, { $set : { 'customerId' : customer._id }});
    }
  });
});
